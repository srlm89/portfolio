...
(define *multiline (lambda (statements prev a)
        (cond ((null? statements) prev)
              (#t (*multiline (cdr statements) (*eval (car statements) a) a)))))
...
(define *apply (lambda (fn x a)
                 (cond ((atom? fn)
                        (cond ((eq? fn (quote car)) (caar x))
                              ...
                              ((eq? fn (quote define)) (define (car x) (cadr x . (cddr x))))
                              ((symbol? fn) (*apply (*eval fn a) x a))
                              (#t fn)))
                       ((eq? (car fn) (quote lambda)) (*multiline (cddr fn) 0 (*pairlis (cadr fn) x a)))
