...
public class EasyNumber {
	...
	public static <T extends Number & Comparable<T>> double median(Collection<? extends T> values) {
		...
		int median = size / 2;
		List<? extends T> sorted = new ArrayList<T>(values);
		Collections.sort(sorted);
		double medianValue = sorted.get(median).doubleValue();
		if (size % 2 == 0) {
			medianValue = (medianValue + sorted.get(median - 1).doubleValue()) / 2.0;
		}
		return medianValue;
	}
}
