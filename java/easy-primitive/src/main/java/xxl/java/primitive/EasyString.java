...
public class EasyString {
	...
	public static String join(Collection<String> subStrings, Character connector) {
		return join(subStrings, String.valueOf(connector));
	}
	...
	public static int longestStringLength(Collection<? extends Object> objects, int lengthOfNullToString) {
		int length = 0;
		for (Object object : objects) {
	...
	public static String repeated(Character character, int times) {
		return repeated(character.toString(), times);
	}
	...
	/**
	 * Returns a plain representation of {@code number}. If the number is an integer, a
	 * trailing decimal symbol with a zero is added. The decimal symbol is always the dot.
	 */
