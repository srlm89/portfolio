...
public class EasyCollection {
	...
	public static <T> Collection<Iterable<? extends T>> asIterables(Collection<? extends T>... collections) {
		Collection<Iterable<? extends T>> asIterables = new ArrayList<Iterable<? extends T>>(collections.length);
	...
	public static <T> Collection<T> copyOf(Collection<T> collection) {
		...
		Collection<T> copy;
		try {
			copy = collectionClass.newInstance();
		}
		catch (Exception e) {
			throw new RuntimeException("Colud not instantiate " + collectionClass, e);
		}
		copy.addAll(collection);
		return copy;
	}

	public static <T> int repetitions(Collection<? extends T> collection, T targetElement) {
		int repetitions = 0;
		boolean isNull = targetElement == null;
		for (T element : collection) {
			if ((isNull && element == null) || (element != null && element.equals(targetElement))) {
				repetitions += 1;
			}
		}
		return repetitions;
	...
	public static <T> T any(Collection<? extends T> collection) {
		for (T element : collection) {
			return element;
		}
		return null;
